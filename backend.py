...
def __cand_print_node( query_cache, index, node_index ) :
...
def __cand_gen_node( query_cache, index, node_index ) :
    query_result = query_cache[1]
    result = query_result[index]
    code = result[0].decode( "utf-8" )
    pinyin = result[1].decode( "utf-8" )

    node_list = result[2]
    node = node_list[node_index]
    hanzi = node[0].decode( "utf-8" )
    
    return [ code, pinyin, hanzi ]

def cand_gen( query_cache, request_length ) :
    cand_list = query_cache[0]
    query_result = query_cache[1]
    #for r in query_result :
        #print r[0], r[1]
    index_list = query_cache[2]
...
def cand_get( query_cache, start_index, length ) :
    if length < 1 :
        length = 0
    cand_list_length = cand_gen( query_cache, start_index + length )
    #for r in cand_list :
    ...
    for i in range( start_index, start_index + length ) :
        if i < cand_list_length :
            node = cand_list[i]
            r.append( __cand_gen_node( query_cache, node[0], node[1] ) )
        else :
            r.append( [ "", "", "" ] )
    return r
    ...
    def query( self, code ):
        ...
        for node in result :
            ...
            for pinyin_node in node[1] :
                ...
                query_cache[1].append( [ code, pinyin, node_list ] )
                query_cache[2].append( 0 )
        return query_cache


    def get_cand( self, query_cache, start_index, length ) :
        return cand_get( query_cache, start_index, length )
